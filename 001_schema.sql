-- ============================================================================
-- Swiss Land Deal-Sourcing Platform — PostGIS schema (Vaud pilot) — NON-DESTRUCTIVE (safe to run every pipeline start)
...
CREATE TABLE IF NOT EXISTS raw.zoning (
    zone_id       text,
    commune_bfs   integer,
...
CREATE TABLE IF NOT EXISTS raw.planning_zones (
    pz_id         text,
    commune_bfs   integer,
    status        text,
    valid_from    date,
...
CREATE TABLE IF NOT EXISTS raw.forest (
    fg_id         text,
    geom          geometry(MultiPolygon, 2056) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS raw.oereb_restrictions (
    egrid         text,
...
CREATE TABLE IF NOT EXISTS audit.parcel_snapshot (
    egrid        text,
